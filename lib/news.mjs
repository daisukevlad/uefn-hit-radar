...
function decode(s) {
  ...
  // Googleニュース等が "&amp;amp;" のように二重エンコードして配信することがあるため、
  // 変化がなくなるまで繰り返しデコードする(通常は1〜2回で収束)
  for (let i = 0; i < 5; i++) {
    const before = s;
...
/**
 * RSS フィード (Google News / Discourseフォーラム) をパース
 * Discourse系(UEフォーラム等)は description内に "N posts - M participants" を含むため
 * 返信数(replies)・参加者数(participants)も抽出する(誰も反応していないスレッドを除外するため)
 * @param {string} xml
 * @param {string} src
 * @returns {Array}
 */
function parseRss(xml, src) {
  ...
  while ((match = itemRegex.exec(xml)) !== null) {
    ...
    // Discourseの "N posts - M participants" から反応の有無を判定
    const engagementMatch = /(\d+)\s+posts?\s*-\s*(\d+)\s+participants?/.exec(item);
    const posts = engagementMatch ? parseInt(engagementMatch[1], 10) : null;
    const participants = engagementMatch ? parseInt(engagementMatch[2], 10) : null;

    if (title && url) {
      items.push({ src, title, url, date, posts, participants });
    }
  }
...
export async function fetchNews() {
  ...
  // フォーラム投稿は「誰かから返信が付いている(参加者2人以上)」もののみ採用。
  // 投稿者本人しか書き込んでいない(参加者1人)スレッドは反応ゼロとみなし除外する。
  // posts/participantsを持たないソース(Reddit・Googleニュース)はそのまま通す。
  allItems = allItems.filter((item) => item.participants == null || item.participants >= 2);
